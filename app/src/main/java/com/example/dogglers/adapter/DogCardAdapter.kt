...
class DogCardAdapter(
    ...
    override fun getItemCount(): Int =
        listDogs.size // TODO: return the size of the data set instead of 0
    ...
    override fun onBindViewHolder(holder: DogCardViewHolder, position: Int) {
        ...
        holder.apply {
            dogImageview?.setImageResource(item.imageResourceId)
            dogNameText?.text = item.name
            dogAgeText?.text = resources?.getString(R.string.dog_age, item.age)
            dogHobbyText?.text = resources?.getString(R.string.dog_hobbies, item.hobbies)
        }
    }
}
